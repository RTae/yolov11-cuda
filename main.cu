...
void process(const std::string &inputPath, const std::string &enginePath, int batchSize, float confidenceThreshold)
{
    ...
    if (isVideo)
    {
        ...
        while (cap.read(frame))
        {
            ...
            if (batchFrames.size() == batchSize)
            {
                ...
                std::cout << "Inference time for batch in " << inputPath << ": " << elapsed << " ms" << ", "<< elapsed/batchSize << "ms/frame" << std::endl;
    ...
    else
    {
        ...
        auto end = std::chrono::high_resolution_clock::now();  // End timing
        double elapsed = std::chrono::duration<double, std::milli>(end - start).count();
        std::cout << "Inference time for image " << inputPath << ": " << elapsed << " ms" << ", "<< elapsed/batchSize << "ms/image" << std::endl;
...
int main(int argc, char **argv)
{
    ...
    {
        threads.emplace_back(process, file, enginePath, batchSize, confidenceThreshold);
    }
